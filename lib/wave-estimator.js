...
const { prevPow2, welchPSD } = require('./fft')
...
function analyze (samples, ctx) {
  ...
  const pp = welchPSD(rs.pitch, rs.fs)
  const rp = welchPSD(rs.roll, rs.fs)
  ...
  // Relative bearing the waves come FROM (0 = dead ahead). Port/starboard side
  // is unresolved, so this is the magnitude folded into [0, pi].
  const directionRelative = following ? (Math.PI - alpha) : alpha
